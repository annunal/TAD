...
#define CURRENT_CPU_T_FILE  "CurrentTempCPU.txt"
...
double CurrentTime_s, LastTimeOut, LastTimeAlert_s, LastTimeConnected_s,LastTimePhoto_s;

float alertSignal1, PhotoTimeInterval;
int alertValue1, PhotoAlertLevel;

//unsigned long ;
int Npoints;
double Tot;

float DtAverage,multFac,addFac, tempCPU;
int backFactor;
int max300 = 100;
int max30 = 10;
int index300 = -1;
...
char alertURL[4096], saveURL[4096], saveURLb[4096],location[4096], title[4096], photoCMD[4096];
...
void initAnalysis() {
	...
	strcpy(saveURLb,Configuration.SaveURLb);
	strcpy(alertURL,Configuration.AlertURL);
  strcpy(photoCMD,Configuration.PhotoCMD);
  PhotoTimeInterval=Configuration.PhotoTimeInterval;
  PhotoAlertLevel=Configuration.PhotoAlertLevel;
  ...
  printf("photoAle=%i\n",PhotoAlertLevel);
	printf("SMSURL=%s\n",Configuration.SMSURL);
 	printf("SMSlist=%s\n",Configuration.SMSlist);
 	printf("SMSuser=%s\n",Configuration.SMSuser);
 	printf("SMSpwd=%s\n",Configuration.SMSpwd);
...
void addMeasure(struct sensorGrid *grid, double time00)
{
	...
	if (CurrentTime_s < LastTimeOut) {
		// sometimes the NTP does not give the right time immediately and give a future time.
		// 1. delete buffer.txt
		// 2. reset Npoints
		// 3. put LastTime to currentTime - 3600

		// 1. delete buffer.txt
		char dummy[4096];
		sprintf(dummy, "sudo rm -f buffer,txt");
		system(dummy);
		// 2. reset Npoints
		index300=-1;
		// 3. put LastTime to currentTime - 3600
		LastTimeOut=CurrentTime_s-3600;
	}
	...
	{ 
		...
		if (index300 >= max300) {
			//printf("0.1  %i %i\n",offset30,offset300);
			if (Configuration.methodInterp == 1) {
				forecast30 = solve1(x300, y300, offset30, max300 , time0, &rms30);
				forecast300 = solve1(x300, y300, offset300, max300, time0,&rms);
			}
			else
			{
		...
		if(*saveURL)
		{
			...
			sprintf(dummy1, "%f",grid->SonarTemp);
			replace(saveURL0,"$V3", dummy1,dummy); strcpy(saveURL0, dummy);
			
			
			//printf ("SaveURL=%s\n",saveURL0);
			t = ctrim(saveURL0);
			// sprintf(dummy, "wget \"%s\" -T 2 -r 1 -nv -o outlogwget.txt", t);
			sprintf(dummy, "curl \"%s\" -o outlogwget.txt", t);

			//printf("calling system: %s\n",dummy);
			system(dummy);
      struct sensorGrid *rowDataDummy ;
      saveAllData(rowDataDummy,1);
			//if ( fileContains("outlogwget.txt","EnterData.aspx"))
      if ( fileContains("outlogwget.txt","OK Nr. 1"))
			{
		...
		if (*alertURL && alertValue>2 && (CurrentTime_s-LastTimeAlert_s)>5*60)
		{
			char alertURL0[4096];
			char dummy[4096];
			char dummy1[4096], *t;
		...
		if (*photoCMD && alertValue>PhotoAlertLevel && (CurrentTime_s-LastTimePhoto_s)>PhotoTimeInterval*60)
		{
			char photoCMD0[4096];
			char dummy[4096];
			char dummy1[4096], *t;
			...
			success = 'T';
			LastTimePhoto_s=CurrentTime_s;
			strcpy(photoCMD0,photoCMD);
			
			replace(photoCMD0,"$DATE",DateString,dummy); strcpy(photoCMD0, dummy);
			replace(photoCMD0,"$TIME",HourString,dummy); strcpy(photoCMD0, dummy);

			sprintf(dummy1,"%3.2f",sensorValue);
			replace(photoCMD0,"$LEV", dummy1,dummy); strcpy(photoCMD0, dummy);

			sprintf(dummy1, "%d",alertValue);
			replace(photoCMD0,"$ALERT_SIGNAL", dummy1,dummy); strcpy(photoCMD0, dummy);
      ...
      if (*Configuration.EmailURL)
      {
          char DateString1[32],HourString1[32];
          //2018-11-04 06:00
   			  strftime(DateString1, sizeof(DateString1), "%Y-%m-%d", gmtm);
			    strftime(HourString1, sizeof(HourString1), "%H:%M", gmtm);

          char emailURL0[4096],body[4096];
          strcpy(emailURL0,Configuration.EmailURL);
          replace(emailURL0,"$TO",Configuration.EmailTo,dummy); strcpy(emailURL0, dummy);

          //[TAD_Server] $IDdevice ($TITLE) ALERT $ALERT_SIGNAL  
          sprintf(dummy, "%d",alertValue);        
          sprintf(dummy1,"[TAD_Server] %s (%s) ALERT %s at %s %s",Configuration.IDdevice,Configuration.title,dummy, DateString1,HourString1);
          replace(emailURL0,"$SUBJ",dummy1,dummy); strcpy(emailURL0, dummy);
          
          // if there is a template use a template
          
          FILE *fh = fopen("./emailTemplate.txt", "rb");
          if ( fh != NULL )
            {
              ...
              fclose(fh); fh = NULL;
              replace(body,"$IDdevice",Configuration.IDdevice,dummy); strcpy(body, dummy);
              replace(body,"$DATE",DateString1,dummy); strcpy(body, dummy);
              replace(body,"$TIME",HourString1,dummy); strcpy(body, dummy);
              replace(body,"$TITLE",Configuration.title,dummy); strcpy(body, dummy);
              sprintf(dummy1,"%3.2f",sensorValue);
              replace(body,"$LEV",dummy1,dummy); strcpy(body, dummy);
              sprintf(dummy1, "%d",alertValue);
              replace(body,"$ALERT_SIGNAL",dummy1,dummy); strcpy(dummy1, dummy);


            }
          else
          {
              sprintf(dummy1, "A new shot email has been done, LEV=%f   ALERT_VALUE=%d  \n\n Picture: https://webcritech.jrc.ec.europa.eu/TAD_server/api/Device/%s/webcam?date=%s%20%s ",sensorValue,alertValue,Configuration.IDdevice,DateString1,HourString1);
          }
          replace(emailURL0,"$CONTENT",dummy1,dummy); strcpy(emailURL0, dummy);
    			//printLog(emailURL0);
    			t = ctrim(emailURL0);
    			sprintf(dummy, "wget \"%s\" -T 20 -nv -o /tmp/TAD/outemailwget.txt &", t);
    
    			printf("calling system: %s\n",dummy);
    			system(dummy);
           
      }
      if (*Configuration.SMSURL)
      {
          char DateString1[32],HourString1[32];
          //2018-11-04 06:00
   			  strftime(DateString1, sizeof(DateString1), "%Y-%m-%d", gmtm);
          ...
          char SMSURL0[4096];
          strcpy(SMSURL0,Configuration.SMSURL);
          replace(SMSURL0,"$SMSUSER",Configuration.SMSuser,dummy); strcpy(SMSURL0, dummy);
          replace(SMSURL0,"$SMSPWD",Configuration.SMSpwd,dummy); strcpy(SMSURL0, dummy);
          replace(SMSURL0,"$SMSLIST",Configuration.SMSlist,dummy); strcpy(SMSURL0, dummy);
          
          sprintf(dummy1,"*** %s, %s Level=%f - ALERT=%i - %s UTC - JRC TAD System",Configuration.location, Configuration.IDdevice,sensorValue,alertValue,HourString1);
          replace(SMSURL0,"$MSG",dummy1,dummy); strcpy(SMSURL0, dummy);
		...
		sprintf(logLine, "%s, %f, %f, %f, %f, %f, %f,  %i, %d, %f, %f, %f, %f, %c\n",
		TimeString,Tot,forecast30,forecast300,rms30,rms,alertSignal,alertValue,Npoints,
		grid->batteryVoltage, grid->panelVoltage, tempCPU, grid->SonarTemp, success);
		printLog(logLine);
		fflush(stdout);
...
float getCPUTemperature()
{
	...
	//printf("Reading file: %s\n",CURRENT_CPU_T_FILE);
	/* Open the file.  If NULL is returned there was an error */
	if(!(infile = fopen(CURRENT_CPU_T_FILE, "r"))) {
