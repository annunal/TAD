...
double CurrentTime, LastTimeGPSSave;
		    ...
		    if(row[0]=='$' && strstr(row,"*") !=0 )
			{
				...
				if(strstr(row,"$GPGGA") !=0)
				{
					...
					{   float latDeg,lonDeg,lonMin,latMin;
						...
						fclose(gpslog);
						//printf("lat-lon=%s  %s\n",res[2],res[4]);
						CurrentTime=(double) time(NULL);  // seconds from 1/1/1970					printf("elev=%s\n",res[9]);
						if(CurrentTime-LastTimeGPSSave>Configuration.gpsDeltaTSave)
						{
							LastTimeGPSSave=CurrentTime;
							char gpsURL0[4096];
							char dummy[4096];
							char dummy1[4096],*t;
